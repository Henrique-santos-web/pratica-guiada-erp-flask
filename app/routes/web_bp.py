...
def editar_produto_view(id):
    ...
    if request.method == "POST":
        ...
        sucesso, mensagem = produtos_controller.salvar_produto(nome, preco, categoria_id, produto_id= id)

        flash(mensagem, "success" if sucesso else "danger")

        if sucesso:
            return redirect(url_for("web.listar_produto_view"))
...
def excluir_produto_view(id):
    sucesso, mensagem = produtos_controller.excluir_produto(id)
    flash(mensagem, "success" if sucesso else "danger")
    return redirect(url_for("web.listar_produtos_view"))
...
def excluir_categoria_view(id):
    sucesso, mensagem = categoria_controller.excluir_categoria(id)
    flash(mensagem, "success" if sucesso else "danger")
    return redirect(url_for("web.listar_categorias_view"))
